import { parseSchemaLinkages, transformToAbsolutePaths } from '../schemaLinkageParser';
...
describe('transformToAbsolutePaths', () => {
  ...
  describe('相对路径转换', () => {
    it('应该转换 dependencies 中的相对路径', () => {
      ...
      const result = transformToAbsolutePaths(linkages, 'contacts.0');
      expect(result['contacts.0.companyName'][0].dependencies).toContain('contacts.0.type');
    });

    it('应该保留绝对路径不变', () => {
      const linkages: Record<string, LinkageConfig[]> = {
        companyName: [{ type: 'value', dependencies: ['#/properties/globalType'] }],
      };

      const result = transformToAbsolutePaths(linkages, 'contacts.0');
      expect(result['contacts.0.companyName'][0].dependencies).toContain('#/properties/globalType');
    });
  });
        ...
        companyName: [
          {
            ...
            when: { field: '#/properties/globalType', operator: '==', value: 'company' },
          },
        ],
      };
