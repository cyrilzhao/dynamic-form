...
describe("arrayLinkageHelper", () => {
  ...
  describe("parseJsonPointer", () => {
    ...
    it("应该对无效的 JSON Pointer 抛出错误", () => {
      expect(() => parseJsonPointer("invalid/pointer")).toThrow(
        "无效的 JSON Pointer",
      );
      expect(() => parseJsonPointer("/properties/name")).toThrow(
        "无效的 JSON Pointer",
      );
    });
  });
  ...
  describe("resolveArrayElementLinkage", () => {
    ...
    it("应该解析 dependencies 中的相对路径", () => {
      ...
      const result = resolveArrayElementLinkage(
        linkage,
