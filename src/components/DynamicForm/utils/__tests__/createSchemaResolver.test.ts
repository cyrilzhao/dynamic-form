...
describe('createSchemaResolver', () => {
  it('校验通过时返回 values 且 errors 为空', async () => {
    ...
    const result = await resolver({ name: 'Alice', email: 'a@b.com' }, undefined, {} as any);
  ...
  it('校验失败时返回嵌套错误结构', async () => {
    const resolver = createSchemaResolver(schema);
    const result = await resolver({ name: '', email: '' }, undefined, {} as any);
  ...
  it('数组子字段错误转换为嵌套结构', async () => {
    ...
    expect((result.errors as any).items?.[0]?.title?.message).toBe('Title is required');
  });
  ...
  it('schema 静态 ui.hidden=true 字段的错误被过滤', async () => {
    const hiddenSchema: ExtendedJSONSchema = {
      ...
      properties: {
        name: { type: 'string', title: 'Name' },
        internalId: { type: 'string', title: 'Internal ID', ui: { hidden: true } },
      },
