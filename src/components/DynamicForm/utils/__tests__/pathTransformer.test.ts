...
describe('pathTransformer (v3.0)', () => {
  ...
  describe('toTemplatePathForCache', () => {
    ...
    it('场景5：当前字段无数组索引时应移除依赖字段的所有索引（覆盖第 157 行）', () => {
      ...
      // 应该移除依赖字段的所有索引
      expect(toTemplatePathForCache('contacts.0.type', 'globalSetting')).toBe('contacts.type');
      expect(toTemplatePathForCache('departments.0.employees.1.name', 'config')).toBe(
