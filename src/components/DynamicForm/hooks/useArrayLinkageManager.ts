 ...
 *
 * 分层职责说明（以数组字段为边界，逐层递归）：
 * - 每层 DynamicForm 只负责本层数组边界以内、下一层数组边界以外的联动
 * - 例如：根级管理根字段和普通嵌套对象字段的联动；departments 数组元素对应的
 *   DynamicForm 管理 departments.0 内部、employees 数组边界以外的联动；以此类推
 * - baseLinkages 中已通过 parentLinkages 过滤掉了外层已负责的联动，不会重复计算
 */
export function useArrayLinkageManager({
  ...
  // v3.1: 支持多联动类型，值为数组格式
  const [dynamicLinkages, setDynamicLinkages] = useState<
   ...
   */
  const generateDynamicLinkages = useCallback((): Record<
    string,
    LinkageConfig[]
  > => {
  ...
  const allLinkages = useMemo(() => {
    ...
    if (!validation.isValid && validation.cycle) {
      console.error(
        "[useArrayLinkageManager] 检测到循环依赖:",
  ...
  const {
    linkageStates,
    refreshLinkage: baseLinkageRefresh,
    setValueWithoutLinkage,
  } = useBaseLinkageManager({
    form,
  ...
  useEffect(() => {
    ...
    const isRefreshCounterChanged =
      refreshCounter !== lastRefreshCounterRef.current && refreshCounter > 0;
