...
describe('跨数组联动测试', () => {
  it('权限数组中有管理员权限时，功能数组中的启用字段应该为true', async () => {
    ...
    // 初始化：普通权限 + 功能
    await act(async () => {
      formRef.current?.setValue('permissions', [{ name: 'View', isAdmin: false }]);
      formRef.current?.setValue('features', [{ name: 'Export', enabled: false }]);
      await new Promise(resolve => setTimeout(resolve, 300));
    });
  ...
  it('移除管理员权限后，功能应该恢复为未启用', async () => {
    ...
    // 初始化：有管理员权限
    await act(async () => {
      formRef.current?.setValue('permissions', [{ name: 'Admin', isAdmin: true }]);
