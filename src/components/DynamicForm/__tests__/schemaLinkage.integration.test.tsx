...
describe('schema 联动集成测试', () => {
  it('应该通过 schema 联动同时更新校验规则、placeholder 和 help 文本', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('Strict mode accepts uppercase letters only')).toBeInTheDocument()
    })
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('Loose mode accepts mixed case')).toBeInTheDocument()
      expect(
