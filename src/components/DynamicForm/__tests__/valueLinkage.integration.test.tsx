...
describe('value 联动集成测试', () => {
  it('应该根据依赖字段自动设置目标字段值', async () => {
        ...
        fullName: {
          ...
          ui: {
            linkages: [
              {
                type: 'value',
                dependencies: ['#/properties/firstName', '#/properties/lastName'],
    ...
    await waitFor(() => {
      expect(getInputByName({ container, name: 'fullName' })).toHaveValue('Ada Lovelace')
    })
    ...
    await waitFor(() => {
      ...
      expect(getInputByName({ container, name: 'fullName' })).toHaveValue('Grace Lovelace')
    })
  })
  ...
  it('value 联动更新的字段应该继续触发下游联动', async () => {
    ...
    const linkageFunctions = {
      getTax: (formData: Record<string, unknown>) => Number(formData.amount ?? 0) * 0.1,
