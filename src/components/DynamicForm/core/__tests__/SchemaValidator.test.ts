  ...
  describe('未覆盖分支补充测试', () => {
    ...
    describe('Line 689: 数组元素 properties 递归验证', () => {
      it('递归验证数组元素的 properties 字段约束', () => {
        ...
        expect(errors['users[0].name']).toBe('users[0].name minimum length is 2 characters');
          ...
          properties: {
            items: {
              ...
              items: {
                type: 'object',
                properties: {
                  value: { type: 'string', title: 'Value', pattern: '^[A-Z]+$' },
                },
              },
            },
          },
    ...
    describe('Line 774: 对象 properties 递归验证', () => {
      it('递归验证嵌套对象的 properties 字段约束', () => {
        ...
        expect(errors['profile.bio']).toBe('profile.bio maximum length is 100 characters');
        expect(errors['profile.website']).toBe('profile.website invalid format (expected: uri)');
      });
      ...
      it('对象 properties 中包含 boolean schema 时应跳过', () => {
        ...
        expect(errors['config.name']).toBe('config.name minimum length is 3 characters');
      });
    });
