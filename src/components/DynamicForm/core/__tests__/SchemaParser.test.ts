...
describe("SchemaParser", () => {
  ...
  describe("getValidationRules - 验证规则", () => {
    describe("必填验证", () => {
      it("应该为必填字段添加 required 规则", () => {
        ...
        expect(typeof fields[0].validation?.validate?.required).toBe(
          "function",
        );
      });
    ...
    describe("长度验证", () => {
      ...
      it("应该使用自定义的长度错误消息", () => {
        ...
        expect(fields[0].validation?.maxLength?.message).toBe(
          "验证码必须是6位",
        );
      });
    });
    ...
    describe("正则表达式验证", () => {
      ...
      it("应该使用自定义的 pattern 错误消息", () => {
        ...
        const fields = SchemaParser.parse(schema);
        expect(fields[0].validation?.pattern?.message).toBe(
          "请输入有效的身份证号码",
        );
      });
    });
  ...
  describe("getValidationRules - 参数默认值测试", () => {
    ...
    it("应该在传递 required=true 时添加 required 规则", () => {
      ...
      expect((rules.validate?.required as Function)(null)).toBe(
