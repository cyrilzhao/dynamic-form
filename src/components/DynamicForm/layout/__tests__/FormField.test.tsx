...
const MockWidget = React.forwardRef<HTMLInputElement, any>((props, ref) => {
  const { name, value, onChange, disabled, readonly, placeholder, error } =
    props;
  return (
    <input
      ref={ref}
...
describe("FormField", () => {
  ...
  describe("Widget 未找到", () => {
    it("当 widget 不存在时应该返回 null 并输出警告", () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        'Widget "non-existent-widget" not found',
      );
  ...
  describe("布局样式", () => {
    ...
    it("horizontal 布局下应该支持数字类型的 labelWidth", () => {
      ...
      const labelWrapperDiv = Array.from(styledDivs).find((div) =>
  ...
  describe("React.memo 优化 (arePropsEqual)", () => {
    ...
    const RenderTrackingWidget = React.forwardRef<HTMLInputElement, any>(
      (props, ref) => {
        renderCount++;
        return (
          <input
    ...
    it("当 field.widget 变化时应该重新渲染", () => {
      ...
      // 改变 widget 类型
      const field2 = createFieldConfig({
        widget: "mock-widget",
        label: "Changed",
      });
      rerender(
  ...
  describe("arePropsEqual 比较函数", () => {
    ...
    it("当 field.schema 不同时应该返回 false", () => {
      ...
      const prevProps = {
        ...baseProps,
        field: { ...baseField, schema: schema1 },
      };
      const nextProps = {
        ...baseProps,
        field: { ...baseField, schema: schema2 },
      };
    ...
    describe("linkageState 比较", () => {
      ...
      it("当 linkageState 内容相同但引用不同时应该返回 true", () => {
        const prevProps = {
          ...baseProps,
          linkageState: { visible: true, disabled: false },
        };
        const nextProps = {
          ...baseProps,
          linkageState: { visible: true, disabled: false },
