...
describe('NumberWidget', () => {
  ...
  describe('基本渲染', () => {
    ...
    it('应该设置正确的 name 属性', () => {
      render(<NumberWidget {...defaultProps} />);
      expect(screen.getByRole('spinbutton')).toHaveAttribute('name', 'number-field');
    });
  ...
  describe('错误状态', () => {
    it('有错误时应该显示 danger intent', () => {
      const { container } = render(<NumberWidget {...defaultProps} error="错误" />);
  ...
  describe('onBlur 回调', () => {
    ...
    it('输入非法字符后失焦应该调用 onChange(undefined) 清空值', () => {
      const handleChange = jest.fn();
      const handleBlur = jest.fn();
      render(<NumberWidget {...defaultProps} onChange={handleChange} onBlur={handleBlur} />);
