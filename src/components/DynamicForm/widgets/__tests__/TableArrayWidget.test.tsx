...
describe("TableArrayWidget", () => {
    ...
    it("应该支持通过 widgetProps 配置虚拟滚动", () => {
      ...
      render(
        <FormWrapper defaultValues={{ items: [{ name: "John", age: 30 }] }}>
          <TableArrayWidget
            name="items"
            schema={schemaWithWidgetProps as any}
          />
        </FormWrapper>,
      );
    ...
    it("schema.items 不是 object 类型时应该返回 null", () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          "schema.items must be an object type with properties",
        ),
      );
  ...
  describe("Widget 类型推断", () => {
    ...
    it("boolean 类型应该使用 checkbox widget", () => {
      render(
        <FormWrapper
          defaultValues={{ items: [{ name: "John", active: true }] }}
        >
          <TableArrayWidget name="items" schema={schemaWithBoolean} />
        </FormWrapper>,
      );
    ...
    it("uri format 应该使用 url widget", () => {
      ...
      render(
        <FormWrapper
          defaultValues={{ items: [{ website: "https://example.com" }] }}
        >
  ...
  describe("未注册的 Widget", () => {
    it("未注册的 widget 应该显示占位符", () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          'Widget "unknown-widget" not found in registry',
        ),
      );
      consoleSpy.mockRestore();
    });
  ...
  describe("列标题", () => {
    ...
    it("没有 title 时应该使用属性名作为列标题", () => {
      ...
      render(
        <FormWrapper
          defaultValues={{ items: [{ firstName: "John", lastName: "Doe" }] }}
        >
