...
describe('EffectEditor', () => {
  ...
  describe('visibility 类型', () => {
    it('应该显示 visible 开关', () => {
      render(
        <EffectEditor {...defaultProps} value={{ state: { visible: true } }} />
      )
  ...
  describe('禁用状态', () => {
    it('禁用状态下添加按钮应该被禁用', () => {
      ...
      expect(
        screen.getByText('Add Test Effect').closest('button')
      ).toBeDisabled()
    })
  })
  ...
  describe('isFulfill 为 false 时', () => {
    it('visibility 类型应该设置 visible 为 false', () => {
      const onChange = jest.fn()
      render(
        <EffectEditor {...defaultProps} onChange={onChange} isFulfill={false} />
      )
