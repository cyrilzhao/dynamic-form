...
import { FieldPathSelector } from '../components/PropertyEditor/components/FieldPathSelector'
...
describe('FieldPathSelector', () => {
  ...
  describe('基础渲染', () => {
    ...
    it('应该显示 placeholder', () => {
      render(
        <FieldPathSelector {...defaultProps} placeholder="Select a field" />
      )
  ...
  describe('对话框交互', () => {
    ...
    it('对话框应该显示路径类型切换按钮', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText('Absolute Path (JSON Pointer)')
        ).toBeInTheDocument()
      })
    })
  ...
  describe('字段过滤', () => {
    it('应该排除当前字段（excludeCurrentField=true）', async () => {
      render(<FieldPathSelector {...defaultProps} excludeCurrentField={true} />)
  ...
  describe('相对路径', () => {
    it('数组元素内部应该显示相对路径选项', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText('Relative Path (Same Level)')
        ).toBeInTheDocument()
      })
    })
  ...
  describe('确认选择', () => {
    ...
    it('未选择路径时确认按钮应该被禁用', async () => {
      render(<FieldPathSelector {...defaultProps} value="" />)
