...
jest.mock('../components/PropertyEditor/components/VariantsEditor', () => ({
  VariantsEditor: ({ onChange }: any) => (
    <div>
      <button
        onClick={() =>
          onChange(
            [{ name: 'text', type: 'string', schema: { type: 'string' } }],
            'text',
          )
        }
      >
      ...
      <button onClick={() => onChange([], undefined)}>Clear Variants</button>
      <button
        onClick={() =>
          onChange(
            [{ name: 'updated', type: 'string', schema: { type: 'string' } }],
            'updated',
          )
        }
      >
...
describe('PropertyEditor', () => {
  ...
  describe('字段级节点', () => {
    ...
    it('首次添加 Variant 时应自动切换到 variant widget', () => {
      const onUpdate = jest.fn()
      render(renderWithContext({ ...defaultContextValue, onUpdate }))
    ...
    it('编辑已有 Variant 时应保持 variant widget', () => {
      const onUpdate = jest.fn()
      const schema: ExtendedJSONSchema = {
        ...
        properties: {
          ...
          name: {
            ...basicSchema.properties!.name,
            ui: {
              widget: 'variant',
              defaultVariant: 'text',
              variants: [{ name: 'text', type: 'string' }],
            },
          },
        },
      }
      render(renderWithContext({ ...defaultContextValue, schema, onUpdate }))

      fireEvent.click(screen.getByText('Variants'))
      fireEvent.click(screen.getByText('Update Variant'))

      expect(onUpdate).toHaveBeenCalledWith(
        ['properties', 'name'],
        expect.objectContaining({
          ui: expect.objectContaining({ widget: 'variant' }),
        }),
      )
    })
    ...
    it.each([
      ...
      '清空 Variant 后应按 %s 字段恢复 %s 默认 widget',
      (fieldName, widget) => {
        const onUpdate = jest.fn()
        const schema: ExtendedJSONSchema = {
          ...basicSchema,
          properties: {
            ...basicSchema.properties,
            [fieldName]: {
              ...basicSchema.properties![fieldName],
              ui: {
                widget: 'variant',
                defaultVariant: 'text',
                variants: [{ name: 'text', type: 'string' }],
        ...
        render(
          renderWithContext({
            ...defaultContextValue,
            schema,
            selectedPath: ['properties', fieldName],
            onUpdate,
          }),
        )

        fireEvent.click(screen.getByText('Variants'))
        fireEvent.click(screen.getByText('Clear Variants'))
    ...
    it.each([
      ...
      ['objectField', 'object', 'nested-form'],
      ['arrayField', 'array', 'array'],
      ['enumField', 'string', 'select'],
    ])('清空 %s 的 Variant 后应恢复默认 widget', (fieldName, type, widget) => {
      const onUpdate = jest.fn()
      const fieldSchema: ExtendedJSONSchema =
        type === 'object'
          ? { type, properties: {} }
          : type === 'array'
      ...
      const schema: ExtendedJSONSchema = {
        ...
        properties: {
          ...
          [fieldName]: {
            ...
            ui: {
              widget: 'variant',
              variants: [{ name: 'variant', type: type as any }],
            },
          },
        },
      }
      ...
      expect(onUpdate).toHaveBeenCalledWith(
        ['properties', fieldName],
        expect.objectContaining({
          ui: expect.objectContaining({ widget }),
        }),
      )
    })
  })
