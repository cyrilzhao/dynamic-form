...
export const PropertyEditor: React.FC = () => {
  ...
  const widgetOptions = {
    string: ['textarea', 'password', 'email', 'url', 'select', 'radio'],
    number: [],
    integer: [],
                ...
                {isObjectProperty && (
                  <Switch
                    ...
                    checked={(() => {
                      ...
                      const parentNode = parentPath.length === 0 ? schema : get(schema, parentPath);
                    ...
                    onChange={e => {
                      ...
                      if (parentNode) {
                        ...
                        onUpdate(parentPath, { required: newRequired.length > 0 ? newRequired : undefined });
                      }
                    }}
                  />
                )}
                            ...
                            {displayEnum.map((value: any, index: number) => (
                              ...
                              >
                                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                  ...
                                  <Tag intent={value ? 'success' : 'none'}>{String(value)}</Tag>
                                </div>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                  <span
                                    style={{
                      ...
                      >
                        {(() => {
                          ...
                          const handleRemoveOption = (index: number) => {
                            const newEnum = enumValues.filter((_: any, i: number) => i !== index);
                            const newEnumNames = enumNames.filter((_: any, i: number) => i !== index);
                            onUpdate(selectedPath, {
                              items: {
                                ...items,
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                              {enumValues.length > 0 ? (
                                ...
                                <Callout intent="warning" icon="info-sign">
                                  No options configured. Add options to enable static array mode.
                                </Callout>
                              )}
                              <Button icon="add" text="Add Option" minimal onClick={handleAddOption} />
                            </div>
                          );
                        })()}
                      </FormGroup>
