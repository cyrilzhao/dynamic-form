...
export const Select: React.FC<SelectProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const initialTerm =
        searchable && !multipleRef.current && selectedOptionsRef.current.length > 0
      ...
      if (onSearchRef.current) {
        ...
        onSearchRef.current(initialTerm).then(results => {
  ...
  // 本地过滤（无 onSearch 时生效）
  const localFilteredOptions = useSearch({ options, searchTerm: onSearch ? '' : searchTerm });
  // 最终展示的选项：有 onSearch 时用异步结果，否则用本地过滤结果
  const filteredOptions = onSearch ? (asyncOptions ?? options) : localFilteredOptions;
  ...
  const handleRemoveTag = useCallback((tagValue: string | number) => {
