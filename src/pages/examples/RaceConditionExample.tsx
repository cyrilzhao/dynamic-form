...
const schema: ExtendedJSONSchema = {
  ...
  properties: {
    ...
    fast: {
      type: 'string',
      title: 'Fast（点击按钮设置为 "yes"，触发 fastField 的快速 visibility 联动 200ms）',
      ui: {
        widget: 'input',
        readOnly: true,
      },
    ...
    fastField: {
      type: 'string',
      title: '⚡ Fast Field（依赖 fast，visibility 联动 200ms）——观察是否被竞态覆盖',
      ui: {
        widget: 'input',
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '900px', margin: '0 auto' }}>
      <Card>
        ...
        <Callout intent="success" title="修复方案：移除自动初始化，改为手动触发" style={{ marginBottom: '16px' }}>
          <p>
            <strong>核心改动：</strong>
            <br />
            • 移除自动初始化的 useEffect
            <br />
            • 暴露 <code>refreshLinkage()</code> 方法供手动调用
            <br />
            • refreshLinkage 会等待 processQueue 完成后再执行，避免并发
          </p>
          <p style={{ marginTop: '8px' }}><strong>测试步骤：</strong></p>
          <ol style={{ paddingLeft: '20px', margin: '4px 0' }}>
            <li>点击"初始化联动"按钮，触发 refreshLinkage（2000ms）</li>
            <li><strong>在 2000ms 内</strong>点击"触发快速联动"按钮</li>
            <li>观察 processQueue 是否等待 refreshLinkage 完成</li>
            <li>观察 <strong>Fast Field 是否正确显示并保持可见</strong></li>
          </ol>
          <p style={{ marginTop: '8px' }}>
            ✅ <strong>预期结果（无竞态）</strong>：Fast Field 最终显示并保持可见
          </p>
        </Callout>
      ...
      <Card style={{ marginTop: '16px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
          ...
          <Button small minimal onClick={() => setLogs([])}>清空</Button>
        </div>
        <p style={{ color: '#666', fontSize: '12px', marginBottom: '8px' }}>
          <span style={{ color: logColors.refresh }}>■ refreshLinkage 路径（2000ms）</span>
          {' · '}
          <span style={{ color: logColors.queue }}>■ processQueue 路径（200ms）</span>
          {' · '}
        ...
        >
          {logs.length === 0 ? (
            ...
            logs.map((log, i) => (
              <div key={i} style={{ color: logColors[log.type], marginBottom: '3px', lineHeight: '1.6' }}>
      ...
      <Card style={{ marginTop: '16px' }}>
        ...
        <div style={{ color: '#555', fontSize: '13px', lineHeight: '1.7' }}>
          <p><strong>修复方案：移除自动初始化，改为手动触发</strong></p>
          <ul style={{ paddingLeft: '20px' }}>
            <li>
              <strong>问题根源：</strong>自动初始化与 processQueue 并发执行，导致状态覆盖
            </li>
            <li>
              <strong>修复方案：</strong>移除自动初始化的 useEffect，暴露 <code>refreshLinkage()</code> 方法
            </li>
            <li>
              <strong>竞态避免：</strong>refreshLinkage 会检查并等待 processQueue 完成后再执行
            </li>
            <li>
              <strong>代码简化：</strong>移除了复杂的 FieldWriteCounter 版本号机制（约150行代码）
            </li>
          </ul>
          <p><strong>新的执行流程：</strong></p>
          <ol style={{ paddingLeft: '20px' }}>
            <li>用户手动调用 <code>refreshLinkage()</code> 初始化联动</li>
            <li>如果 processQueue 正在运行，refreshLinkage 等待其完成</li>
            <li>如果 refreshLinkage 正在运行，processQueue 等待其完成</li>
            <li>两者串行执行，不会互相覆盖</li>
          </ol>
          <p><strong>优势：</strong>代码更简洁，逻辑更清晰，易于维护，完全避免竞态问题。</p>
        </div>
      </Card>
    </div>
  );
