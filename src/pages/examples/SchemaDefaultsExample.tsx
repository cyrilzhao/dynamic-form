          ...
          linkages: [
            {
              ...
              when: { field: 'content.priority', operator: '==', value: 'high' },
              fulfill: { state: { visible: true } },
              otherwise: { state: { visible: false } },
            },
          ],
          ...
          linkages: [
            {
              ...
              when: { field: 'content.enableDebug', operator: '==', value: true },
              fulfill: { state: { visible: true } },
              otherwise: { state: { visible: false } },
            },
          ],
                ...
                linkages: [
                  {
                    ...
                    when: { field: 'advancedSettings.enableLogging', operator: '==', value: true },
                    fulfill: { state: { visible: true } },
                    otherwise: { state: { visible: false } },
                  },
                ],
    ...
    <Card style={{ marginTop: '20px', maxWidth: '900px' }}>
      ...
      <Callout intent="primary" style={{ marginBottom: '20px' }}>
        ...
        <ul style={{ marginBottom: '10px' }}>
          <li>
            <Tag intent="success">静态默认值</Tag> Schema 中的 <code>default</code>{' '}
            值在表单初始化时自动设置
          </li>
          <li>
            <Tag intent="warning">联动计算</Tag> 默认值参与联动计算，控制字段的显示/隐藏状态
          </li>
          <li>
            <Tag intent="primary">动态 Schema</Tag> 通过 schema 联动加载的新 schema，其 default
            值也会被自动应用
          </li>
        </ul>
      </Callout>
      ...
      <Callout intent="none" icon="info-sign" style={{ marginBottom: '20px' }}>
        ...
        <ol style={{ marginBottom: 0, paddingLeft: '20px' }}>
          <li>观察表单初始化时的默认值（Action Name = "New Action"，Enable Action = true 等）</li>
          <li>注意 "Run on Startup" 字段的可见性随 "Enable Action" 变化</li>
          <li>选择一个 Action Type，观察动态加载的 schema 及其默认值</li>
          <li>点击 "Show Current Values" 查看完整的表单数据（包含所有默认值）</li>
        </ol>
      </Callout>
