...
export const MemoPerformanceTest: React.FC = () => {
  ...
  const schema = useMemo(() => {
    ...
    const generatedSchema = generateLargeScaleSchema(
      arrayItemCount,
      fieldsPerItem
    )
      ...
      if (lastChangeTime.current > 0) {
        ...
        if (responseTime < 1000) {
          ...
          const avgResponseTime =
            recentResponseTimes.current.reduce((a, b) => a + b, 0) /
            recentResponseTimes.current.length
      ...
      {enableMonitoring && (
        ...
        >
          ...
          <Callout intent="primary" style={{ marginBottom: '15px' }}>
            <strong>Test Instructions:</strong> Type in the first field of the
            first array item to see the performance impact. With React.memo
            optimization, only the modified field should re-render.
          </Callout>
          <div
            style={{
              display: 'grid',
