...
export const LargeDataPerformanceExample: React.FC = () => {
  ...
  return (
    <div style={{ padding: '20px' }}>
      ...
      <p>
        This example tests the performance of dynamic forms with large amounts
        of complex data.
      </p>
      ...
      >
        ...
        <div style={{ marginTop: '10px', color: '#666' }}>
          ...
          >
            Note: Hidden fields still affect form state management and linkage
 ...
 * 生成复杂的测试 Schema
 */
function generateComplexSchema(
...
function generateNestedStructure(
  ...
  if (currentLevel < maxLevel) {
    properties[`level${currentLevel + 1}`] = {
      ...
      properties:
        generateNestedStructure(maxLevel, currentLevel + 1).properties || {},
    }
  }
...
function generateNestedDefaults(maxLevel: number, currentLevel: number): any {
  ...
  const data: any = {
    ...
    [`level${currentLevel}Type`]: ['option1', 'option2', 'option3'][
      currentLevel % 3
    ],
  ...
  if (currentLevel < maxLevel) {
    data[`level${currentLevel + 1}`] = generateNestedDefaults(
      maxLevel,
      currentLevel + 1
    )
  }
 ...
 * 生成类型特定字段的默认值
 */
function generateTypeSpecificDefaults(
  type: string,
  fieldCount: number
): Record<string, any> {
