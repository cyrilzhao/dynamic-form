...
const fetchActionSchema = async (
  ...
  console.log(
    `[API Call #${callNumber}] Fetching schema for action: ${actionId}`
  )
  ...
  return (
    <Card style={{ marginTop: '20px', maxWidth: '900px' }}>
      ...
      <Callout intent="primary" style={{ marginBottom: '20px' }}>
        ...
        <p>
          本示例演示了异步联动和结果缓存功能。当切换 <code>actionId</code>{' '}
          下拉菜单时，
          <code>content</code> 字段的 schema 会动态变化。
        </p>
        <ul style={{ marginBottom: '10px' }}>
          <li>
            <strong>异步联动</strong>：schema 从服务器异步加载（模拟 400-1200ms
            延迟）
          </li>
          <li>
          ...
          <li>
            <strong>结果缓存</strong>：启用 <code>enableCache: true</code>，相同
            actionId 不会重复请求
          </li>
        </ul>
      ...
      <Callout intent="none" icon="info-sign" style={{ marginBottom: '20px' }}>
        ...
        <ol style={{ marginBottom: 0, paddingLeft: '20px' }}>
          ...
          <li>
            快速连续切换不同的 Action Type，观察控制台日志（竞态条件处理）
          </li>
          <li>
            切换回之前选过的 Action Type，观察 API Calls
            计数（缓存生效时不会增加）
          </li>
