...
export const ArrayWithFlattenExample: React.FC = () => {
  ...
  return (
    <div>
      <H3>数组字段 + 路径透明化</H3>
      <p>
        这个示例展示了如何在数组元素内部使用路径透明化特性。外层的{' '}
        <code>group</code> 和 <code>category</code> 被扁平化，数组元素内部的{' '}
        <code>category.group</code> 也被扁平化。
      </p>
      ...
      <Card style={{ marginTop: '20px' }}>
        ...
        <ul>
          <li>
            <strong>外层路径透明化</strong>：<code>region</code> 和{' '}
            <code>market</code> 设置了 <code>flattenPath: true</code>，不会渲染
            Card 边框
          </li>
          <li>
            <strong>数组字段</strong>：<code>contacts</code> 由 ArrayFieldWidget
            处理，支持增删操作
          </li>
          <li>
            <strong>数组内部路径透明化</strong>：每个数组元素内部的{' '}
