...
export const RefreshLinkageExample: React.FC = () => {
  ...
  const linkageFunctions: Record<string, LinkageFunction> = {
    ...
    getDepartmentOptions: () => {
      console.log(
        '[getDepartmentOptions] 被调用, departments.length:',
        departments.length
      )
    ...
    getPermissionOptions: (formData, context) => {
      ...
      if (!role || permissions.length === 0) {
        console.log(
          '[getPermissionOptions] role 为空或 permissions 为空，返回空数组'
        )
        return []
      }
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      <Callout intent="primary" style={{ marginBottom: '20px' }}>
        <h4>功能说明</h4>
        <p>
          本示例演示如何在异步数据加载完成后，使用 <code>refreshLinkage()</code>{' '}
          重新触发表单联动。
        </p>
        <ul style={{ marginBottom: 0 }}>
          <li>
            <strong>场景 1 - 普通字段联动：</strong>
            选择部门后，员工列表会根据部门筛选
          </li>
          <li>
            <strong>场景 2 - 嵌套表单联动：</strong>
            选择认证类型后，配置列表会根据类型筛选
          </li>
          <li>
        ...
        >
          {loading ? (
            ...
            <>
              ...
              <span>
                数据已加载{' '}
                {dataLoadedAt && `(${dataLoadedAt.toLocaleTimeString()})`}
              </span>
            </>
          )}
        </div>
